...
<br>
<footer class="text-muted"> <div class="container"> <p class="float-right"> <a href="#">Powrót do góry</a> </footer>
  `;
	$('body').html(html);
}
...
<input class="form-control mr-sm-2" name="q" type="search" placeholder="" aria-label="Search" value="${search_text}" required>
...
function list(path) {
	var content = `
<p style="fontsize: 20px"><center>Ostatnia aktualizacja: 08.05.2021</center></p>
...
function render_search_result_list() {
  ...
  <div class="container"><br>
  <div class="card">
  <h5 class="card-header">Wynik wyszukiwania</h5>
  <div id="list" class="list-group">
  </div>
  </div>
  <div class="card">
...
function onSearchResultItemClick(a_ele) {
  ...
  console.log('cancel');
		...
		dialog.close();
		dialog = mdui.dialog({
			title: 'Nie udało się znaleźć ścieżki pliku',
			content: 'Mogło coś się zbugować, lub jest problem z uprawnieniami',
			history: false,
			modal: true,
			closeOnEsc: true,
			buttons: [{
